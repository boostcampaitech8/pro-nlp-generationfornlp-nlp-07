...
MARGIN_THRESHOLD = float(os.environ.get("MARGIN_THRESHOLD", "1.0"))
# - 애매 문항에서 샘플 몇 번 할지
SC_SAMPLES = int(os.environ.get("SC_SAMPLES", "3"))
# - 샘플링 온도
SC_TEMPERATURE = float(os.environ.get("SC_TEMPERATURE", "0.8"))
# - 애매 문항 비율이 너무 커지는 것을 방지(상한)
MAX_SC_RATE = float(os.environ.get("MAX_SC_RATE", "0.25"))  # 전체의 최대 25%까지만 SC 실행
# - seed 고정(재현성)
