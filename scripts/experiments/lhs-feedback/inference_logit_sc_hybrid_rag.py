...
def main():
    ...
    RAG_DENSE_MODEL = os.environ.get("RAG_DENSE_MODEL", "intfloat/multilingual-e5-base")
    ...
    def _get_retriever():
        ...
        if retriever is not None:
            return retriever
        try:
            from retriever_fast_cpu_compat import HybridWikiRetriever
            retriever = HybridWikiRetriever(
                ...
                dense_model=RAG_DENSE_MODEL,
                reranker_model=(RAG_RERANKER_MODEL if RAG_RERANKER_MODEL else None),
                dense_device=RAG_DENSE_DEVICE,
                rerank_device=RAG_RERANK_DEVICE,
            )
            return retriever
        except Exception as e:
            rag_disabled = 1
    ...
    for i, row in enumerate(pbar, start=1):
        ...
        if need_rag and RAG_RUN == 1 and (not rag_disabled):
            ...
            try:
                ...
                rag_failures += 1
                if RAG_DEBUG and rag_failures <= 5:
                    print(f"[WARN] RAG failed id={qid} err={e}")
