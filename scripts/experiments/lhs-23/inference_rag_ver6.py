...
def main():
    ...
    def _get_retriever():
        ...
        try:
            # BM25 버전 리트리버로 교체
            from retriever_hybrid_faiss_bm25 import HybridWikiRetriever
            ...
            retriever = HybridWikiRetriever(
                ...
                bm25_tf_path=os.environ.get("RAG_BM25_TF", "bm25_tf.npz"),
                bm25_meta_path=os.environ.get("RAG_BM25_META", "bm25_meta.npz"),
                embed_model_name=RAG_DENSE_MODEL,
                device=RAG_DENSE_DEVICE,
                reranker_name=(RAG_RERANKER_MODEL if RAG_RERANKER_MODEL else None),
            )
            return retriever
        except Exception as e:
            rag_disabled = 1
    ...
    for i, row in enumerate(pbar, start=1):
        ...
        if need_rag and RAG_RUN == 1 and (not rag_disabled):
            ...
            try:
                ...
                if RAG_DEBUG_CHOICE and rag_used < RAG_DEBUG_CHOICE_MAX:
                    ctx, dbg = build_choice_aware_ctx(
                        question=question,
                        ...
                        per_choice_sents=3,
                        debug=True,
                        debug_topn=RAG_DEBUG_CHOICE_TOPN,
                    )
                    # choice-wise debug print
                    print(f"\n[RAG-CHOICE] i={i}/{total} id={qid}")
                    for ci in range(1, len(choices)+1):
                        print(f"  - Choice {ci}: {choices[ci-1][:60]}")
                        for (sc, title, sent) in dbg.get(ci, [])[:RAG_DEBUG_CHOICE_TOPN]:
                            print(f"      * sc={sc:.2f} [{title}] {sent[:120]}")
                else:
