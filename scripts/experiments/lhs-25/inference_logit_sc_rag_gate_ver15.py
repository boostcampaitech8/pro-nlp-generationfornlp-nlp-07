...
def build_rag_context(
    ...
    *,
    ...
    for d in docs:
        ...
        # ✅ 문서 점수: 상위 문장 점수 합(또는 max/avg로 바꿔도 됨)
...
def main():
    ...
    for i, row in enumerate(pbar, start=1):
        ...
        if use_rag:
            # retrieve candidate docs (no rerank)
            ...
            if ctx:
                ...
                pred_rag, m_rag, e_rag, p_rag = logits_label_stats(
                    model, tokenizer, rag_prompt, allow_labels, max_len
                )

                # accept only if improves over ORIGINAL base stats (stable rule)
                improve = (m_rag > margin) or (p_rag > top1p) or (e_rag < ent)
                if improve:
                    pred = pred_rag
                    did_rag = True
