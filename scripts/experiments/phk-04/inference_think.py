...
# 추론 설정
parser.add_argument('--inf_batch_size', type=int, default=1, help='추론 배치 크기 (기본값: 1)')
...
bad_token_ids = []
# Vision pad 차단
if '<|vision_pad|>' in vocab:
    bad_token_ids.append(vocab['<|vision_pad|>'])
    print("⚠️ <|vision_pad|> 토큰 차단")

# 기타 불필요한 토큰 차단
for bad_token in ['<|vision_start|>', '<|vision_end|>', '<|image_pad|>']:
    if bad_token in vocab:
        bad_token_ids.append(vocab[bad_token])

answer_token_ids = [vocab[str(i + 1)] for i in range(5)]

def parse_thinking_response(response):
    if '</think>' in response:
        after_think = response.split('</think>')[-1]
        answer = re.search(r'[1-5]', after_think)
        if answer:
            return int(answer.group()) - 1
        
    return None

def fallback_logits_extraction(output_ids, len_choices):
    with torch.no_grad():
        logits = model(output_ids.unsqueeze(0)).logits[0, -1, :]
    
    target_logits = logits[answer_token_ids[:len_choices]]
...
model.eval()
with torch.no_grad():
    for i in tqdm(range(0, len(test_dataset), INFERENCE_CONFIG['batch_size']), desc="Thinking Inference"):
        # 배치 데이터 준비
        ...
        outputs = model.generate(
            **inputs,
            ...
            suppress_tokens=bad_token_ids if bad_token_ids else None,
        )

        # 첫 샘플로 디버깅
        if i == 0:
            print("첫 샘플로 디버깅 수행")
            debug_decode = tokenizer.decode(outputs[0], skip_special_tokens=False)
            print("   Model Answer:")
            print(debug_decode)
        ...
        for j in range(len(batch_texts)):
            ...
            # <think> 태그 안의 정답 추출 시도
            pred_idx = parse_thinking_response(response)

            # 추출 실패 시 logits 기반 예측으로 대체
            if pred_idx is None or pred_idx >= len_choices:
                pred_idx, probs = fallback_logits_extraction(output_ids, len_choices)
                parsing_method = "logits"
            else:
                _, probs = fallback_logits_extraction(output_ids, len_choices)
                parsing_method = "regrex"
            ...
            detailed_results.append({
                ...
                'full_response': response if i < 5 else None,  # 처음 5개 샘플만 전체 응답 저장
            })
...
for r in detailed_results:
    ...
    parsing_stats[method] = parsing_stats.get(method, 0) + 1

print(f"\n파싱 방법 통계:")
for method, count in parsing_stats.items():
    print(f"  {method}: {count}/{len(detailed_results)} ({count/len(detailed_results)*100:.1f}%)")
