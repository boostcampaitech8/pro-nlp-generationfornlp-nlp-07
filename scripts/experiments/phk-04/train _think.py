...
# 데이터 설정
parser.add_argument('--train_data', type=str, default='train_with_cot.csv', help='학습 데이터 파일 이름 (기본값: train_with_cot.csv)')
...
# 설정된 instruction_part, response_part를 스페셜 토큰으로 등록, 정말 필요한 경무에만 사용
# 해당 부분 사용 시 모델 사이즈가 달라져, 모델 병합 과정이 추가로 필요해짐
# special_tokens = {
...
def parse_data(example):
    ...
    return {
        ...
        'cot_text': example.get('cot_text', None),
    }
...
def build_messages(data, include_answer):
    ...
    if include_answer:
        messages.append({"role": "assistant", "reasoning_content": data['cot_text'], "content": str(data['answer'])})
...
def apply_chat_template_safe(tokenizer, messages, include_answer, tokenize=False):
    ...
    #     text += CHAT_TEMPLATE_CONFIG['response_part'] + "<think>\n"
    
    # if len(messages) == 2:
    #     text += messages[1]['reasoning_content'] + "\n</think>" + messages[1]['content'] + tokenizer.eos_token
...
# 학습 내용 검사
if '<think>' in learning_text and '</think>' in learning_text:
    print("✅ Thinking 형식 검증 성공")

    answer_part = learning_text.split('</think>')[-1].strip()
    print(f"   최종 답변 부분: {answer_part[0]}")
    if answer_part or answer_part[0] in ["1", "2", "3", "4", "5"]:
        print(f"✅ 최종 답변 검증 성공: {answer_part[0]}")
    else:
        print("⚠️ 학습 내용 검증 실패 ('</think>'이후 토큰이 1~5 사이의 값이 아님)")
        print("마스킹 전 전체 텍스트 (디버그 용):")
        print(tokenizer.decode(batch['input_ids'][0], skip_special_tokens=False))
        sys.exit(1)
else:
    print("⚠️ 학습 내용 검증 실패 (<think> 형식이 없음)")
    print("마스킹 전 전체 텍스트 (디버그 용):")
    print(tokenizer.decode(batch['input_ids'][0], skip_special_tokens=False))
    sys.exit(1)
