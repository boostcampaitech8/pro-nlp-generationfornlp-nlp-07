...
# If tfidf_vectorizer.joblib was saved when KoTokenizer lived in __main__,
# joblib.load will look for __main__.KoTokenizer. We inject it here so
# existing artifacts load without forcing a rebuild.
try:
    from tokenizers_ko import KoTokenizer  # recommended module location
    import __main__
...
class HybridWikiRetriever:
    def __init__(
        ...
        self.dense = SentenceTransformer(dense_model, device=device or ("cuda" if torch.cuda.is_available() else "cpu"))
        self.dense.eval()
