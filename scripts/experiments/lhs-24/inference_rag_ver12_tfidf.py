...
def build_retrieval_query(
    ...
    chs = [str(c).strip() for c in (choices or [])[:5] if str(c).strip()]
    ch_block = " ; ".join(chs)

    s = (q + "\n" + qp + "\n" + ch_block).strip()
    knowledge_like = bool(re.search(
        r"(뜻|의미|용어|개념|정의|어원|한자|문법|사상|인물|작가|작품|왕|연도|조약|제도|법|불교|유교|철학)",
        s
    ))
    ...
    parts = []
    if include_para:
        parts.append(para[:para_chars])
    parts.append(q)
    if qp:
        parts.append(qp[:160])
    if ch_block:
        parts.append(ch_block[:220])
