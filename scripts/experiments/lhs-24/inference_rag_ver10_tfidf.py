...
def get_reranker(model_name, device="cpu"):
    global _RERANKER, _RERANKER_TOK
...
def rerank_with_bge(query, docs, model_name, top_n=5, device="cpu"):
    """
    docs: List[dict], each has 'text'
    return: reranked docs[:top_n]
    """
    if not docs:
        return docs
    ...
    docs = sorted(docs, key=lambda x: x["_rerank_score"], reverse=True)
    print(f"[RAG-RERANK] reranking {len(docs)} docs")
    return docs[:top_n]
...
def main():
    ...
    for i, row in enumerate(pbar, start=1):
        ...
        prompt0 = fit_prompt_to_max_len(tokenizer, prompt0, max_len)
        ...
        if need_rag and RAG_RUN == 1 and (not rag_disabled):
            ...
            try:
                ...
                # optional rerank (BGE reranker, cross-encoder)
                if int(RAG_USE_RERANK) == 1 and docs:
                    try:
                        ...
                        if RAG_DEBUG and rag_attempts <= RAG_DEBUG_MAX:
                            print(f"[RAG-RERANK-FAIL] id={qid} type={type(e).__name__} {e}")
                ...
                if not ctx:
                    ...
                    prompt_rag = fit_prompt_to_max_len(tokenizer, prompt_rag, max_len)
