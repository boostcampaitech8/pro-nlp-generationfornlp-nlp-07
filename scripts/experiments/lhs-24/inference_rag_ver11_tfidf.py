...
def main():
    ...
    RAG_RERANKER_MODEL = os.environ.get("RAG_RERANKER_MODEL", "BAAI/bge-reranker-v2-m3").strip()
...
# load model/tokenizer
    ...
    if RAG_TRACE == 1:
        ...
        print(f"[RAG-TRACE] enabled path={RAG_TRACE_PATH}")
    ...
    def _dump_trace(rec: dict):
        ...
        # q_emb 같은 비직렬/대용량 키는 제거 (필요하면 shape만 남김)
        if isinstance(rec, dict) and "call_kwargs" in rec and isinstance(rec["call_kwargs"], dict):
            ck = dict(rec["call_kwargs"])
            if "q_emb" in ck:
                q = ck.pop("q_emb")
                try:
                    import numpy as np
    ...
    for i, row in enumerate(pbar, start=1):
        ...
        # --- per-sample RAG temporaries (avoid UnboundLocal) ---
        rq = None
        docs = []
        prompt_rag = None
        ...
        if need_rag and RAG_RUN == 1 and (not rag_disabled):
            ...
            try:
                ...
                # ----- OPTIONAL: cross-encoder rerank (logs controlled by RAG_RERANK_LOG) -----
                # Note: retriever may also implement its own rerank; this is an extra safety layer.
                if RAG_USE_RERANK == 1 and isinstance(docs, list) and len(docs) > 0:
                    ...
                    if str(rr_device).startswith("cuda") and (not torch.cuda.is_available()):
                        rr_device = "cpu"
                    try:
                        # rerank only a bounded number of candidates to limit cost
                        cand_n = max(RAG_K_FINAL, min(len(docs), max(RAG_K_FUSE, RAG_K_SPARSE, RAG_K_DENSE, 50)))
                        cand = docs[:cand_n]
                        ...
                        # keep reranked first, then append the rest to preserve any downstream expectations
                        docs = reranked + docs[cand_n:]
                    except Exception as _e:
                        if RAG_DEBUG and rag_attempts <= RAG_DEBUG_MAX:
                            print(f"[RAG-RERANK-WARN] type={type(_e).__name__} repr={repr(_e)}")
