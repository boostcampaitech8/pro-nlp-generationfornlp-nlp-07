...
export EXP_NAME="ENS_rag_gate_ver13_tfidf"
...
export RAG_K_DENSE=50
export RAG_K_SPARSE=50
export RAG_K_FUSE=100
export RAG_K_PRE_RERANK=30
export RAG_K_FINAL=5
export RAG_RERANKER_MODEL=BAAI/bge-reranker-v2-m3

# 컨텍스트 길이(노이즈 줄이기)
export RAG_CTX_MAX_CHARS=1500

# paragraph 섞는 건 기본 off 권장(지문형 문제에서 검색 튐 방지)
export RAG_USE_PARAGRAPH=0
...
export RAG_RERANK_LOG=1

# RAG 트레이스 파일 저장
export RAG_TRACE=1
export RAG_TRACE_PATH="./rag_traces_ver13.jsonl"

# 어떤 샘플을 저장할지: all | flagged | accepted | nonempty
export RAG_TRACE_MODE="flagged"
export RAG_TRACE_MAX=200

# prompt/ctx 저장 길이 제한(너무 커지는 것 방지)
export RAG_TRACE_PROMPT_MAX_CHARS=12000
export RAG_TRACE_DOC_TEXT_MAX_CHARS=600
...
for ADP in "${ADAPTER_MODELS[@]}"; do
  ...
  nohup python3 -u inference_rag_ver12_tfidf.py > "${LOG_OUT}" 2>&1
