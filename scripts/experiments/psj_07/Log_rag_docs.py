...
LOG_CSV_PATH = "scripts/experiments/psj_07/rag_used_docs_log.csv"
...
class KiwiBM25Retriever:
    ...
    def _tokenize(self, text: str):
        ...
        # 혹시 위 품사 필터로 전부 날아가면 백업으로 전체 사용
        if not tokens:
...
def main():
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="log_rag_docs"):
        ...
        # (2) confidence가 threshold 이상이면 RAG 안 씀 → 스킵
        if base_conf >= CONF_THRESHOLD:
            continue

        print(f"[RAG-USED] idx={idx}, id={row.get('id')}, "
              f"base_pred={base_pred}, base_conf={base_conf:.3f}")
