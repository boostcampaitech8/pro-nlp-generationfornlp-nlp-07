...
export EXP_NAME="Qwen2.5-32B-Instruct-persona"
...
# ===== KoNET-style Persona + Silent CoT (추론 품질 ↑, 출력은 1토큰 유지) =====
# base(로짓 추론) 페르소나
export BASE_PERSONA="teacher"
# SC 수행 시에만 다양한 페르소나로 1토큰 샘플링
export SC_USE_PERSONA=1
export SC_PERSONAS="teacher,student,professor,academic"
# 출력은 정답(1~5)만 유지하면서 내부 점검을 유도하는 1줄 힌트
export SILENT_COT=1

nohup python3 -u inference_logit_sc_persona.py > "${INFER_LOG}" 2>&1 &
